...
function invest_all() {
	...
	if (balance > 0.00000001) {
			investations = parseFloat($(".investment").html().replace(/,/g, ""));
			balance = parseFloat($("#pct_balance").val());
			balance = balance.toFixed(8);
...
function stop_low() {
	var investations = parseFloat($(".investment").html().replace(/,/g, "")); //parseFloat($('#stopUpper1').val() >= investations)
	if ($('#divest_upper').prop('checked') && $stopLower1.val() >= parseFloat(investations) ) {
        //console.log('stop.low Divested' +  $stopLower1.val() + investations);
            if (parseFloat(investations) > 0.00000001){
                divest_all();
            }
        return;
    }
...
function stop_high() {
	var investations = parseFloat($(".investment").html().replace(/,/g, "")); //investations >= parseFloat($('#stopLower1').val()
	if ($('#divest_lower').prop('checked') && parseFloat(investations) >= $stopUpper1.val()) {
        //console.log('stop.high Divested' + $stopUpper1.val() + investations);
            if (parseFloat(investations) > 0.00000001){
                divest_all();
            }
        return;
...
function divest_all() {
	...
	balance = balance.toFixed(8);
	if (balance < 0.00000001) {
		var investations = parseFloat($(".investment").html().replace(/,/g, ""));
...
function updateStats() {
	if (start_check == 1) {
		...
		$(".Bot_Profit").html('Ð' + bot_profit.toFixed(8));
...
*/
	...
	var $whalePSe = $('<p style="margin-right:15px;border:1px solid; border-color: #505050;" class="rlabel">Ð</p>');
	...
	var $profitPSe = $('<p style="margin-right:15px;border:1px solid; border-color: #505050;" class="rlabel">Ð</p>');
	...
	var $profitPS1e = $('<p style="margin-right:15px;border:1px solid; border-color: #505050;" class="rlabel">Ð</p>');
	...
	var $stopUpper = $('<p style="border:1px solid; border-color: #505050;" class="llabel">stop Upper</p>');
	  $stopUpper1 = $('<input style="border:1px solid; border-color: #505050;" id="stopUpper1" value="0"  />');
	var $stopUpper2 = $('<p style="margin-right:15px;border:1px solid; border-color: #505050;" class="rlabel">Ð</p>');
	  $row3.append($stopUpper);
	  $row3.append($stopUpper1);
	$row3.append($stopUpper2);

	var $stopLower = $('<p style="border:1px solid; border-color: #505050;" class="llabel">stop Lower</p>');
	  $stopLower1 = $('<input style="border:1px solid; border-color: #505050;" id="stopLower1" value="0"  />');
	var $stopLower2 = $('<p style="margin-right:15px;border:1px solid; border-color: #505050;" class="rlabel">Ð</p>');
	  $row3.append($stopLower);
	  $row3.append($stopLower1);
	$row3.append($stopLower2);
	...
	//divest_upper
	$divest_upper = $('<div><input type="checkbox" value="1" name="divest_upper" id="divest_upper" /> Stop Upper active</div>')
		$o_row1.append($divest_upper);
        
	//divest_lower
	$divest_lower = $('<div><input type="checkbox" value="1" name="divest_lower" id="divest_lower" /> Stop Lower active</div>')
		$o_row1.append($divest_lower);

	/////////////////////////////////////////////////////////////////////////////////////

	var $graphDiv = $('<fieldset id="chipper3" style="margin-left:auto;margin-right:auto;margin-top:5px;background-color:rgba(35,35,35,0.9);border:2px solid; border-color: #999999;width:700px;height:100px;margin-right:3px" class="graph-container"><div style="padding: 0;width:700px;height:100px;margin-right:0px" id="g_placeholder" class="graph-placeholder"></div>');  //profit graph holder
	var $profit_label = $("<div align='center' style='color:white;font-size:8pt;'>Profit</div>");
	var $graphDiv2 = $('<fieldset id="chipper4" style="margin-left:auto;margin-right:auto;background-color:rgba(35,35,35,0.9);border:2px solid; border-color: #999999;width:700px;height:100px;margin-right:3px" class="graph-container2"><div style="padding: 0;width:700px;height:100px;margin-right:0px" id="g2_placeholder" class="graph-placeholder"></div>'); //invested graph holder
	...
	$button_group.append("<div align='center' style='color:white;font-size:8pt;'>Doge-Dice --- Investor-Stats</div>");
